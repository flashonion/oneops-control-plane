...
export function createApp(config: AppConfig) {
  const app = express()
  const store = new SnapshotStore(createDemoConnectors(), config.SNAPSHOT_TTL_MS)
  ...
  app.get('/api/v1/connectors', async (_request, response, next) => {
    try {
      ...
      response.json({ data: snapshot.data.connectors, stale: snapshot.stale, ageMs: snapshot.ageMs })
    } catch (error) {
      next(error)
    }
  })
